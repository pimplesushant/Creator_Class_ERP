...
class Add_staff_model extends CI_Model {
	...
	function count_all(){
	 ...
	 function save($add_staff_ctrl){
		$this->db->insert($this->tbl_faculty, $add_staff_ctrl);
	...
	// update person by id
	function update($id,$add_staff_ctrl){
		$this->db->where('faculty_id',$id);
		$this->db->update($this->tbl_faculty,$add_staff_ctrl);
	} 
}
?>