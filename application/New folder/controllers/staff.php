...
class Staff extends CI_Controller 
{
	...
	function index($offset = 0)
	{
		...
		$this->table->set_heading('No', 'Name', 'Gender', 'Date of Birth','Telephone No','mobile No','Email','Address', 'View','Update','Delete');
		$i = 0 + $offset;
		foreach ($persons as $person){
			...
			$person->telephone_no,$person->mobile_no,$person->email_id,$person->address,
				anchor('staff/view/'.$person->faculty_id,'view',array('class'=>'view')),
				anchor('staff/update/'.$person->faculty_id,'update',array('class'=>'update')),
				anchor('staff/delete/'.$person->faculty_id,'delete',array('class'=>'delete','onclick'=>"return confirm('Are you sure want to delete this person?')"))
			);
		}
		$data['table'] = $this->table->generate();
		
		$data['action'] = site_url('staff/addstaff');
		$data['link_back'] = anchor('staff/index/','back to list of persons',array('class'=>'back'));
	...
	function update($id){
		...
		$data['message'] = '';
		$data['action'] = site_url('staff/updatestaff');
		$data['link_back'] = anchor('staff/index/','Back to list of persons',array('class'=>'back'));
